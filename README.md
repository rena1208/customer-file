...
## ●苦労した点
### ○お客様の一覧の中から頭文字を選択し絞り込む機能
JavaScriptのfilter()メソッドに濁音・半濁音を変換する処理、charAt()メソッドを組み合わせて処理を書く部分が苦労しました。<br>
接客時にもスムーズに使用できるように、LaravelではなくJavaScriptで実装したのですが、<br>
JavaScriptでの開発が慣れていないこともあり苦戦しましたがconsole.log()でログを細かく出しながら<br>
少しずつ処理を書くよう意識して実装いたしました。

### ○使用状況シート（カレンダー風のUIのページ）
こちらの実装は、こういったカレンダー風のUIの実装をしているような記事や情報がない中での実装で手元で確認しながら<br>
試行錯誤を繰り返したため大変苦労いたしました。<br>
商品の購入年月をリレーションや、SQL文でデータを作成する部分はdd()を使用しながら意図したデータが取れているか確認しながら実装、<br>
カレンダー風のUIの部分はif文やfor文を使用してカテゴリーごとに表示、購入のない場合は空欄にすることによって実現ができました。
