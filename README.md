...
JavaScriptのfilter()メソッドに濁音、半濁音文字を変換する処理、charAt()メソッドを組み合わせて処理を書く部分が苦労しました。
接客時にもスムーズに使用できるように、LaravelではなくJavaScriptで実装したのですが、JavaScriptでの開発が慣れていないこともあり
苦戦しましたがconsole.log()でログを細かく出しながら少しずつ処理を書くよう意識して実装いたしました。

### ○使用状況シート（カレンダー風のUIのページ）
こちらの実装は、こういったカレンダー風のUIの実装をしているような記事や情報がない中での実装で手元で確認しながら試行錯誤を繰り返したため
大変苦労いたしました。
商品の購入年月をリレーションや、SQL文でデータを作成する部分はdd()を使用しながら意図したデータが取れているか確認しながら実装、
